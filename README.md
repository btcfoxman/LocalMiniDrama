...
前往 **[Releases 下载页](https://github.com/btcfoxman/LocalMiniDrama/releases)**：
...
| 层 | 技术 |
|----|------|
| 前端 | Vue 3 + Vite + Element Plus + Pinia + Axios + @vue-flow/core |
...
**最新版 v1.2.11 亮点：**
- 🔧 **打包版默认本地存储**：内置存储配置改为本地模式，不再预填 3.6 S3；上传、编辑保存和生成结果默认保存在本机
- 🔧 **参考图 base64 请求**：本地模式下调用模型时自动读取本地参考图并转成 base64 或 multipart 请求，不再上传 S3 或外部图床
