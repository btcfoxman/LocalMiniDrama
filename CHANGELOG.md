...
- **打包版默认本地存储**：内置 `storage` 默认改为 `local`，不再预填 3.6 S3 endpoint、bucket 与密钥；首次安装和未自定义的旧 3.6 配置会随升级迁移到本地模式
- **存储设置支持本地 / S3 切换**：AI 配置页的「存储设置」改为模式选择，默认本地，用户需要远程对象存储时再主动切到 S3 并填写自己的配置
- **本地参考图模型请求 base64 化**：本地模式下，图片/视频生成的参考图会从 `/static/...` 或本地相对路径读取并转为 base64 或 multipart 请求，不再上传 S3，也跳过外部图床中转
- **保留自定义 S3**：已经在软件内保存过自定义 S3 的用户，后续升级不会被本地默认配置覆盖

### 发布

- `frontweb` / `backend-node` / `desktop` 的 `package.json` 与各自 `package-lock.json` 顶层 **version** 统一为 **1.2.11**
...
- **自定义 S3 存储配置**：AI 配置面板新增「存储设置」，支持用户自行填写 S3 Endpoint、Bucket、Region、AK/SK、公开访问 Base URL、Cloudflared 签名 Host，并可一键测试签名上传
- **默认 3.6 存储保留**：默认仍使用 `https://s3-3-6.aiid.edu.kg` 与 `signing_host: 192.168.3.6:9000`，适配 3.6 Cloudflared + RustFS 部署

### 优化

- **桌面端升级不覆盖自定义 S3**：打包版启动时只同步默认/旧 3.6 存储配置，用户在软件内保存过自定义 S3 后，后续升级不会再被内置配置覆盖
- **资源图片操作按钮布局**：缩小「AI 生成 / 上传 / 编辑」按钮间距和内边距，避免新增的「编辑」按钮在资源卡片底部被挤出可视区域
- **S3 连接缓存刷新**：S3 客户端缓存签名加入 Bucket 和 Secret，修改 Bucket 或密钥后不会继续复用旧客户端

### 发布

- `frontweb` / `backend-node` / `desktop` 的 `package.json` 与各自 `package-lock.json` 顶层 **version** 统一为 **1.2.10**
...
- **图片编辑器**：媒体素材库、角色、场景、道具图片支持打开编辑器后保存为新图或设为主图
- **打码工具**：支持套圈空白、涂抹擦拭、网状线覆盖、马赛克区域，适合处理人物脸部等容易触发真人审核的区域
- **基础修图**：支持裁剪、左右旋转、查看缩放、导出缩放、PNG/JPG 导出与 JPG 质量调节

### 优化

- 媒体素材库上传图片后自动登记为素材，列表搜索支持按名称、URL、local_path 查询，并返回文件大小与图片尺寸

### 发布

- `frontweb` / `backend-node` / `desktop` 的 `package.json` 与各自 `package-lock.json` 顶层 **version** 统一为 **1.2.9**
...
- **桌面 exe 生成视频后假死**：视频远端同步改为流式落盘，ffmpeg 画幅归一化从 `spawnSync` 改为异步子进程，避免阻塞 Electron 主进程事件循环
- **视频本地回写/上传内存占用**：本地存储不再重复整文件读写，S3 上传改用文件流，降低大视频同步时的卡顿和内存峰值

### 发布

- `frontweb` / `backend-node` / `desktop` 的 `package.json` 与各自 `package-lock.json` 顶层 **version** 统一为 **1.2.7**

## [upstream-1.2.8] - 2026-07-01
